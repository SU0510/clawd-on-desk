...
//
// v0 → v1: add `version`, `agents`, `themeOverrides` fields.
// v1 → v2: Hermes added to agent defaults.
// v2 → v3: raise passive notification bubble default from 3s to 6s.
// v3 → v4: remove agents, remote SSH, telegram approval, permission bubbles,
//   session aliases, codex permission mode, and related fields.
function migrate(raw) {
  ...
  if ((out.version || 0) < 4) {
    delete out.agents;
    delete out.manageClaudeHooksAutomatically;
    delete out.autoStartWithClaude;
    delete out.bubbleFollowPet;
    delete out.permissionBubblesEnabled;
    delete out.permissionBubbleAutoCloseSeconds;
    delete out.notificationBubbleAutoCloseSeconds;
    delete out.updateBubbleAutoCloseSeconds;
    delete out.tgApproval;
    delete out.remoteSsh;
    delete out.sessionAliases;
    out.version = 4;
  }
...
function load(prefsPath) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`Dommy: prefs file unreadable, backed up to ${bak}:`, err.message);
    } catch (bakErr) {
      console.warn("Dommy: prefs file unreadable and backup failed:", err.message, bakErr.message);
    }
    return { snapshot: getDefaults(), locked: false };
  }
  ...
  if (incomingVersion > CURRENT_VERSION) {
    console.warn(
      `Dommy: prefs file version ${incomingVersion} is newer than supported (${CURRENT_VERSION}). ` +
